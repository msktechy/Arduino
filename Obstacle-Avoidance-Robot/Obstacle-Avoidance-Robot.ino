...
const int led    = 7;     //Led pin (throught resistor) to pin 3
const int buzzer = 4;     //Buzzer pin to pin 4
const int enA= 3;
const int motorA1= 5;     //motor A positive (+) pin to pin 6 (PWM) (from L298 module!)
const int motorA2= 6;         //motor A negative (-) pin to pin 9 (PWM)
const int enB= 11;
const int motorB1=9;     //motor B positive (+) pin to pin 10 (PWM)
const int motorB2=10;     //motor B negative (-) pin to pin 11 (PWM)
...
void setup()
{
    myservo.attach(8);      //Servo pin connected to pin 5
  ...
  if (function == 0){   //If button is unpressed or pressed twice then:
    ...
    if (distance > 10){
      ...
      for(pos = 0; pos < 180; pos += 1){  //goes from 0 degrees to 180 degrees 
            ...
            else if (checkLeft <=10 && checkRight <=10v,keeeeeeeeeeee){
